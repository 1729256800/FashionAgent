...
你的任务:
1.由于网页内容可能是英文，你的输出必须是 **中文**，但是其中的一些**专有名词**可以是英文。
...
你现在是一个时尚分析员,请你根据总结的网页信息对相关领域的时尚或着装趋势进行分析，并提供时尚推荐
"""
Content_FA = """
需求如下
{topic}
...
对于这个要求
1.输出语言为中文，专有名词可用原语种（但要有词的解释）
2.请根据信息你分析时尚的趋势，并结合需求，提出一些时尚的搭配的建议
3.总结的信息可能存在语意不通的情况，请根据全文适当总结，提取精华
4.可以分段（不要分点）进行回答，说出“具体会穿什么款式衣服，以什么颜色为主”或“一个配饰，由什么材料制成，形如什么”，但要注意需求不要偏题
5.大概,400字左右，语言精炼
6.不要有空格
"""
...
class FashionAna(Action):
    async def run(self, topic, require_data: dict[str, dict[str, str]], *args, **kwargs):
        """
        ...
        prompt = Content_FA.format(topic=topic, require=require_text)
        print("进入大模型")
...
class FashionResearcher(Role):
    ...
    async def _act(self) -> Message:
        ...
        if isinstance(todo, CollectLinks):
            ...
            content = await todo.run(topic, summaries)
            ret = Message(
